...
const year2000 = 1000 * 60 * 60 * 24 * (365 * 30 + 7) + msTimezone;
...
const generateTransactions = number => {
  ...
  for (let i = 0; i < number; i += 1) {
    ...
    if ((tooMuch && total > 0) || doExpense) {
      ...
      transactions.push(expense(amount, currentDate));
      total -= amount;
    } else {
      // Income
      const amount = randomAmount(QUANTITY);
      transactions.push(income(amount, currentDate));
      total += amount;
    }
  }
  return transactions;
